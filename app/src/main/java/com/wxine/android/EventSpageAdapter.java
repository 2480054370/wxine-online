 ...
 */
public class EventSpageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public EventSpageAdapter(Context context, List<Info> list) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == ITEM_TYPE.ITEM_TYPE_IMAGE.ordinal()) {
            return new ImageViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.event_s_top, parent, false));
        } else {
            return new TextViewHolder(context,LayoutInflater.from(parent.getContext()).inflate(R.layout.event_s_item, parent, false));
        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ImageViewHolder) {
            ...
            if (a.getComments().size() > 0) {
                ViewGroup.LayoutParams lp = ((TextViewHolder) holder).Pinglun_item_recycler.getLayoutParams();
                lp.height = 200;
                ((TextViewHolder) holder).Pinglun_item_recycler.setLayoutParams(lp);
                ((TextViewHolder) holder).Pinglun_item_recycler.requestLayout();
                final CommentsAdapter mAdapter = new CommentsAdapter(context, new ArrayList<Comment>(a.getComments()));
                ((TextViewHolder) holder).Pinglun_item_recycler.setAdapter(mAdapter);
                ...
                if (count > 1) {//一个滚动没有意义
                    ...
                    Runnable runnable = new Runnable() {
                        ...
                        @Override
                        public void run() {
                            ((TextViewHolder) holder).Pinglun_item_recycler.smoothScrollToPosition(to);
                            if (to >= count) to = 1;
                            else to = to + 1;//重新开始
    ...
    public static class TextViewHolder extends RecyclerView.ViewHolder {
        ...
        public RecyclerView Pinglun_item_recycler;
        ...
        TextViewHolder(Context context,View view) {
            ...
            Pinglun_item_recycler = (RecyclerView) view.findViewById(R.id.pinglun_recycler_view);
            Pinglun_item_recycler.setHasFixedSize(true);

            FullyLinearLayoutManager mLayoutManager = new FullyLinearLayoutManager(context);
            Pinglun_item_recycler.setLayoutManager(mLayoutManager);
    ...
    public static class ImageViewHolder extends RecyclerView.ViewHolder {
        ...
        ImageViewHolder(final View view) {
            ...
            button = (Button) view.findViewById(R.id.event_button);
            content = (TextView) itemView.findViewById(R.id.CollS_name);
